const NintendoVendorId = 0x057e;
const JoyConLProductId = 0x2006;
const JoyConRProductId = 0x2007;
const DefaultRumble = [0x00, 0x01, 0x40, 0x40, 0x00, 0x01, 0x40, 0x40];

const CRCTABLE8 = [
  ]

/**
 * CRC-8-CCITTの計算を行う。
 * @param {number[]} data もととなるデータ
 * @returns {number} 計算結果
 */
const calcCrc8 = (data) => {
  let val = 0
  let pos = 0
  data.forEach(element => {
    val = CRCTABLE8[val ^ element]
  });
  return val
}

/**
 * deviceに向けてoutput reportを送信する。
 * @param device 対象となるHID device
 * @param {number} reportId 
 * @param {number[]} packetNumber 
 * @param {number} rumbleData 
 * @param {number} subCommand 
 * @param {number[]} argument 
 */
const writeOutputReport = (device, reportId, packetNumber, rumbleData, subCommand = 0x00, ...argument) => {
  let n = Uint8Array.from([packetNumber].concat(rumbleData).concat([subCommand]).concat(argument));
  device.sendReport(reportId, n);
}

/**
 * MCUに対し、CRC-8を含む0x01のoutput reportを送信する。
 * @param device 対象となるHID device
 * @param {number} packetNumber 
 * @param {number[]} rumbleData 
 * @param {number} subCommand 
 * @param {number} mcuCommand 
 * @param {number} mcuSubCommand 
 * @param {number[]} argument 
 */
const writeOutputReport0x01Crc = (device, packetNumber, rumbleData, subCommand, mcuCommand, mcuSubCommand, ...argument) => {
  let mcuSubCmdArg = [mcuSubCommand].concat(argument);
  for(let i = 0; i<35-argument.length; i++){
    mcuSubCmdArg.push(0);
  }
  let buf = [packetNumber].concat(rumbleData).concat([subCommand, mcuCommand]).concat(mcuSubCmdArg);
  buf.push(calcCrc8(mcuSubCmdArg));
  device.sendReport(0x01, Uint8Array.from(buf));
}

/**
 * MCUに対し、CRC-8を含む0x11のoutput reportを送信する。
 * @param device 対象となるHID devices
 * @param {number} packetNumber 
 * @param {number[]} rumbleData
 * @param {number} mcuCommand 
 * @param {number} mcuSubCommand 
 * @param {number[]} {number[]} argument 
 */
const writeOutputReport0x11Crc = (device, packetNumber, rumbleData, mcuCommand, mcuSubCommand, ...argument) => {
  let mcuSubCmdArg = [mcuSubCommand].concat(argument);
  for(let i = 0; i<35-argument.length; i++){
    mcuSubCmdArg.push(0);
  }
  let buf = [packetNumber].concat(rumbleData).concat([mcuCommand]).concat(mcuSubCmdArg);
  buf.push(calcCrc8(mcuSubCmdArg));
  device.sendReport(0x11, Uint8Array.from(buf));
}

const useHID = () => {
  const { ref, computed, onMounted, watch } = Vue

  const devices = ref([null, null])

  const connectHID = async () => {
    const device = (await navigator.hid.requestDevice({
      filters: [{
        vendorId: NintendoVendorId,
      }]
    }))[0]

    if (device) {
      devices.value.splice(device.productId === JoyConLProductId ? 0 : 1, 1, device)
      await device.open()
    }
  }

  return {
    devices,
    connectHID
  }
}